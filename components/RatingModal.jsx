...
const RatingModal = ({ ratingModal, setRatingModal }) => {
    ...
    const handleSubmit = async () => {
        if (rating < 0 || rating > 5) {
            return toast('Silakan pilih rating');
        }
        if (review.length < 5) {
            return toast('Tulis ulasan singkat');
        }
        try {
            ...
            dispatch(fetchProducts({})) // Refresh products to update ratings
    ...
    return (
        <div className='fixed inset-0 z-120 flex items-center justify-center bg-black/10'>
            <div className='bg-white p-8 rounded-lg shadow-lg w-96 relative'>
                ...
                <h2 className='text-xl font-medium text-slate-600 mb-4'>Beri Rating Produk</h2>
                ...
                ></textarea>
                <button onClick={e => toast.promise(handleSubmit(), { loading: 'Mengirim...' })} className='w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition'>
                    Kirim Rating
                </button>
            </div>
        </div>
